...
| **FR-2** | Dynamic Scaling & Memory | Users can adjust the serving size for any recipe. The system recalculates quantities instantly and allows saving preferences as a personal default. | Serving count<br>(1-12) | Scaled quantities,<br>Persistent defaults | **(Happy Path)** Given a recipe for 2 people, when the user changes servings to 10 and clicks 'Save', then the new default is stored in localStorage.<br>**(Edge Case)** Given the serving input, when the user enters '0', then the system must default to '1'. | Must |
| **FR-3** | Persistent Planner | Users schedule meals across a 7-day calendar. Each planned meal can have its own independent serving count, affecting the weekly budget and grocery list. | Recipe ID, Date,<br>Serving count | Backend-persisted<br>weekly schedule | **(Happy Path)** Given a selected recipe, when it is dropped into a slot, then the slot must display the recipe and allow +/- serving adjustments.<br>**(Persistence)** Given a completed plan, when the page is refreshed, then the schedule and custom portions must be re-loaded from the database. | Must |
...
| **NFR-4** | Portability | System must support rapid deployment and local testing via Docker containers on any OS with Docker support. | Image size < 200MB | Run `docker images mealmate-frontend` to verify compressed size. | Should |
