...
- **Possible Solution**: Migrate all `storageService` dependencies to the backend API so that user settings are fully portable.
...
- **Possible Solution**: Refactor into smaller, stateless UI components and move business logic into custom hooks (e.g., `useRecipes`, `useMealPlan`).
...
- **Possible Solution**: Add a dedicated Vitest suite for `src/engines/`.
...
- **Possible Solution**: Implement a simple interceptor in `apiClient.js` to catch 401 Unauthorized errors and redirect to login.
